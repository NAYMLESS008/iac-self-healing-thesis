...
# --- Scenario paths and control-result destination ---
...
# --- Remove cron test artefacts between control conditions ---
def clean():
    run_target_command(
...
# --- Read both Wazuh-agent service state and count FIM-ready log markers ---
def get_status_and_marker_count():
    ...
    # Parse the two explicit STATUS=/COUNT= lines from the remote command.
...
# --- Get the cron artefact creation/modification timestamp for alert-delay calculation ---
...
# --- Inject the same controlled cron persistence used in the main experiment ---
def run_attack():
...
# --- Return the newest Wazuh FIM alert that exactly matches the cron artefact ---
...
# --- Poll until a matching alert with a new ID appears or timeout expires ---
...
# --- Convert the Wazuh ISO timestamp into epoch seconds for delay calculation ---
...
# --- Mark control alerts processed so they do not interfere with later runs ---
...
# --- Append one paired-control repetition to CSV ---
def save_row(row):
...
# --- Run one paired A/B repetition around Wazuh FIM readiness ---
def run_pair(rep):
    print("\n")
    print("=" * 60)
    ...
    # Condition A injects the cron artefact after the agent reports active
    # but before a NEW real-time FIM-ready marker appears.
    ...
    # Restarting the agent creates a fresh transition from service-active to FIM-ready.
    run_target_command(
    ...
    # First measure how quickly systemd reports the Wazuh agent as active.
    while True:
    ...
    # If FIM was already ready, this repetition cannot test the intended pre-ready window.
    ...
    # True means the attack completed before a new FIM-ready marker appeared.
    ...
    # Measure the later point where real-time FIM reports itself ready.
    while True:
    ...
    # This is the measured service-active -> FIM-ready gap.
    gap = round(
    ...
    # Wait for the alert generated by the pre-readiness artefact.
    ...
    # Condition B injects the same cron artefact after FIM is already ready.
    ...
    # --- Save the paired timing/detection result for this repetition ---
    row = {
        "repetition": rep,
...
# --- This script completes the second and third paired repetitions ---
